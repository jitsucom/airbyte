...
public class MongoUtils {
    ...
    public static JsonSchemaType getType(final BsonType dataType) {
        return switch (dataType) {
            ...
            case STRING, SYMBOL, BINARY, DATE_TIME, TIMESTAMP, OBJECT_ID, REGULAR_EXPRESSION, JAVASCRIPT ->
    ...
    private static ObjectNode readField(final BsonReader reader,
        ...
        switch (fieldType) {
            ...
            case TIMESTAMP ->
                    o.put(fieldName, DataTypeUtils.toISO8601StringWithMilliseconds(reader.readTimestamp().getValue()));
    ...
    public static List<TreeNode<CommonField<BsonType>>> getUniqueFields(final MongoCollection<Document> collection) {
        var first = collection.find().first();
        final var allkeys = first != null ? first.keySet() : Collections.<String>emptySet();

        return allkeys.stream().map(key -> {
            var v = first.get(key);
            BsonType tp = STRING;
            if (v != null) {
                tp = switch (v.getClass().getName()) {
                    case "java.lang.Integer" -> INT32;
                    case "java.lang.Long" -> INT64;
                    case "java.util.Date" -> DATE_TIME;
                    default -> STRING;
                };
            }
            final var fieldNode = new TreeNode<>(new CommonField<>(key, tp));
